...
export default class EachMyAuthPendingServiceDetail extends React.Component {
  genrateResponse = () => {
  ...
  render() {
    ...
    this.props.data.userList.forEach(data => {
      if (data.name == retrievedUserDetails.name && data.status == false) {
        flag = true;
      }
    });
      ...
      <Delete color={red500} onTouchTap={this.approve}/>
          ...
          <CardActions>
            {this.props.data.proofReq ? (
              <FlatButton label="Response" onTouchTap={this.genrateResponse} />
            ) : null}
